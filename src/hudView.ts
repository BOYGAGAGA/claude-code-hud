...
export class HudViewProvider implements vscode.WebviewViewProvider {
  ...
  private _pinnedFile: string | null = null;
  private _dailySpend: any[] = [];
  private _thresholds: any = {};
  ...
  public update(
    ...
    dailySpend?: any[],
    thresholds?: { warning?: number; limit?: number }
  ) {
    this._stats = stats;
...
function buildHtml(s: any, sessions: any[], pinnedFile: string | null, dailySpend: any[], thresholds: any): string {
  ...
  const doneTodos = s.todos.filter((t: any) => t.status === 'completed').length;
  const activeTodos = s.todos.filter((t: any) => t.status === 'in_progress').length;
  const totalPct = totalTodos > 0 ? Math.round((doneTodos / totalTodos) * 100) : 0;

  // Session selector — clickable title buttons
  const sessionButtons = sessions.map((sess: any) => {
  ...
  // Agent groups
  const agentGroups: any[] = s.agents || [];
  const totalAgentTasks = agentGroups.reduce((a: number, g: any) => a + g.totalCount, 0);
  const doneAgentTasks = agentGroups.reduce((a: number, g: any) => a + g.doneCount, 0);

  const todoRows = s.todos.map((t: any, i: number) => {
...
<div class="section">
  ...
  if (dailySpend.length > 0) {
    ...
    const todayCost = dailySpend.find((d: any) => d.date === today)?.cost ?? 0;
    const weekTotal = dailySpend.filter((d: any) => d.date !== today).reduce((a: number, b: any) => a + b.cost, 0);
    const maxCost = Math.max(...dailySpend.map((x: any) => x.cost), 0.001);
    ...
    const prevDays = dailySpend.filter((d: any) => d.date !== today).slice(-6).reverse();
    const prevMaxCost = Math.max(...prevDays.map((x: any) => x.cost), 0.001);
    const rows = prevDays.map((d: any) => {
...
<div class="section">
  ...
  <div class="section-body">
    ${agentGroups.map((agent: any) => {
      const agentColor = agent.pct >= 100 ? '#4ade80' : agent.pct > 0 ? '#60a5fa' : '#334155';
      const agentTasksHtml = agent.tasks.map((t: any) => {
        const hasProgress = typeof t.progress === 'number';
