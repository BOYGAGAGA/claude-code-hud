...
// Data sourced from official pricing pages as of mid-2025
  ...
  try {
    ...
    for (const line of content.split('\n')) {
      ...
      try {
        ...
        if (typeof c === 'string' && c.trim()) candidate = c.trim();
        if (Array.isArray(c)) {
          for (const item of c) {
...
function encodePath(workspacePath: string): string {
...
export function calcDailySpend(workspacePath: string, config: CostConfig, days = 7): Array<{ date: string; cost: number }> {
